...
private struct PrivacyPane: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                Toggle("On-device AI features", isOn: $appleAI)
                    .disabled(!AppleAI.isAvailable)
                ...
                Toggle("Group tabs by topic", isOn: $tidyTabs)
                    .disabled(!appleAI || !AppleAI.isAvailable)
                Toggle("Rename messy downloads", isOn: $tidyDownloads)
                    .disabled(!appleAI || !AppleAI.isAvailable)
                Toggle("Shorten pinned tab titles", isOn: $tidyTitles)
                    .disabled(!appleAI || !AppleAI.isAvailable)
            } footer: {
                Text(AppleAI.unavailableReason
